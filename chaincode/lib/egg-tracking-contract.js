...
const EXTERNAL_API_SERVER = "https://e94c-195-169-74-58.eu.ngrok.io";
...
class EggTrackingContract extends Contract {
	...
	async packEggs(ctx, farmerId, packingTimestamp, quantity) {
		...
		let key =
			eggBox.getType() + ":" + eggBox.getOriginId() + ":" + eggBox.getPackingTimestamp();
	...
	async createShipment(
		...
		let key =
			shipment.getType() +
			":" +
			shipment.getFarmerId() +
			":" +
			shipment.getDistributorId() +
			":" +
			shipment.getShipmentCreation();
		...
		if (eggBoxes.length < min) {
			throw new Error(
				`Insufficient amount of egg boxes ${eggBoxes.length} for the shipment. Minimum amount is ${min}`
			);
		}
	...
	async loadBoxes(ctx, shipmentId, loadTimestamp) {
		...
		const event = {
			...
			targetAudience: [
				eggShipment.getFarmerId(),
				eggShipment.getShipperId(),
				eggShipment.getDistributorId(),
			], // suggested targetAudience
	...
	async getFarmerStamp(id, timestamp) {
		try {
			...
			const json = await axios(
				`${EXTERNAL_API_SERVER}/farmer/${id}?packingTimestamp=${timestamp}`
			);
